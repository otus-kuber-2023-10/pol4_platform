...
8.1 Подготовка объекта мониторинга.
...
Особенно мила невозможность полноценной отладки, т.е. с ноды, где работает nginx нет доступа к сервису контенера через lh.
Что, конечно, ожидаемо и понятно, но очень неудобно.

При создании сервиса важно публиковать не только сам порт nginx, но и порт Exporter. Сделано откровенно криво, т.к. это нужно только для его обнаружения
прометеем через настройки Service Monitor... Доступ идёт по внутренним IP контейнеров сервиса, а не через кластерный IP.
Фактически, это заставляет его открыть чтобы потом закрыть на МСЭ...

Также при настройке именно deployment nginx обязательно нужно настроить анотации для обнаружения подов с nginx и запуска скрейпер-job-ов для сбора с endpoints (с порта Exporter-а)
...
8.2 Сбока и запуск объекта мониторинга

Собрал и опубликовал образ Docker с Nginx.
Сделали деплой этого образа с 3-мя реприками
Запустил сервис.
Проверил наличие правильных меток и аннотаций. (app= и annotation)

Всё про объект мониторинга живёт, как и положено в отдельном неймспейсе - demo

done

8.3 Ставим прометея через helm вместе с графаной.

Ставим в отдельный неймспейс - monitoring

done

8.4 Заставляем прометея увидеть объект мониторинга как Target и собрать с него метрики
...
- Создаём ServiceMonitor в неймспейсе monitoring, для обнаружения которого прометеем нужно прописать магические аннотации и лейблы, которые стоит подсмотреть в заведомо рабочих сервисмониторах конктетной
инталляции прометея.
В моём случае это:
  annotations:
...
Чтобы найденный прометеем сервисмонитор мог точно невести на наш сервис сборщик метриц прометея нужно прописать ему:
spec:
  namespaceSelector:
    matchNames:
    - demo # Где искать
...
8.5 Проверяем что оно таки увидело
...
Хорошо подходит метрика nginx_http_requests_total, т.к. для пустого nginx без нагрузки бутет показывать число запросов от прометея на сбор метрик по нодам.
Т.е. паттерн нормальной работы - когда метрики на нодах в разные моменты в то отличаются не более чем на 1, то равны.
Что и наблюдаем.
